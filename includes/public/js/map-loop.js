...
export function cttmMapLoop(doc, shortcode) {
  ...
  if (shortcode.options.max_cluster_radius) {
    var max_cluster_radius = parseInt(shortcode.options.max_cluster_radius);
  } else {
  ...
  let container = doc.getElementById('travelersmap-container-' + containerid);
  ...
  if (shortcode.metas != 0) {
    ...
    for (let i = 0; i < shortcode.metas.length; i++) {
      ...
      if (markerdatas) {
        ...
        //If "this_post" option is set and not a multimarker post
        //Add the marker in our cluster group layer without popover
        //Else add it with its popover
        if (shortcode.options.this_post == 'true' && !markerdatas.multiplemarkers) {
        ...
        if (
          ...
          (global_options['only_main_marker'] === 0 ||
            shortcode.options.this_post === 'true')
        ) {
            
          for (let index = 1; index < markerdatas.multiplemarkers; index++) {
            let postdatas = Object.assign({}, shortcode.metas[i].postdatas);
            //Get markerdatas object
            let markerdatasMultiple = markerdatas['additional_marker_' + index];
            //Initialize all markers variables
